import { buildBrowseTreeWithProgress, type ProgressSchedulerRecord } from '../../domain/phase6/progress';
import type {
...
export class Phase6OpeningTrainerRepository extends Phase6ContrastRecallRepository {
  public override async initialize(now = nowIso()): Promise<void> {
    ...
    await this.enqueue(() => this.migrateLegacyOpeningNames(now));
  }
  public getScopeQueueSummary(scope: TrainingScope, now = new Date()): Promise<ScopeQueueSummary> {
    return this.enqueue(async () => {
      const moveRows = await this.moveCandidateRows(scope, 'normal', now, 'queue-summary');
      let due = 0;
      let fresh = 0;
      ...
      const nameStates = new Map((await this.database.nameSchedulerStates.toArray()).map((row) => [row.itemId, row]));
      let namesDue = 0;
      let namesNew = 0;
      ...
      return { due, new: fresh, contrast: confusions.filter((row) => row.contrastDue).length, namesDue, namesNew };
    });
  }
  public browseWorkspace(
    ...
    return this.enqueue(async () => {
      ...
      const { graph, playlist } = await this.scopeGraph(scope);
      const allScopeIds = await this.scopeRepertoireIds(scope);
      const repertoireId = options.repertoireId && allScopeIds.includes(options.repertoireId)
      ...
      const selectedContextId = options.contextId && contexts.some((row) => row.id === options.contextId)
        ? options.contextId
        : repertoire.rootContextIds[0];
      if (!selectedContextId) throw new Error('Repertoire has no root context.');
      const selectedContext = graph.contexts.find((row) => row.id === selectedContextId);
      const selectedPosition = selectedContext ? graph.positions.find((row) => row.id === selectedContext.entryPositionId) : undefined;
      if (!selectedContext || !selectedPosition) throw new Error('Selected Browse position is missing.');
      const items = (await this.database.trainingItems.toArray()).filter((row) => row.repertoireId === repertoireId);
      const stateRows = await this.database.schedulerStates.toArray();
      const schedulerRows: ProgressSchedulerRecord[] = stateRows
        .filter((state) => items.some((item) => item.id === state.trainingItemId))
        .map((state) => ({ trainingItemId: state.trainingItemId, state: state.state, retrievability: this.scheduler.retrievability(state.state, now) }));
      const playlistEligible = playlist
        ? new Set(contexts.filter((row) => playlistAllowsContext(graph, playlist, row)).map((row) => row.id))
        : undefined;
  ...
  protected async getScopeQueueSummaryUnsafe(scope: TrainingScope, now: Date): Promise<ScopeQueueSummary> {
    const moveRows = await this.moveCandidateRows(scope, 'normal', now, 'queue-summary');
    let due = 0;
    let fresh = 0;
    for (const row of moveRows) {
  ...
  public async listImportHistory(): Promise<ImportRecord[]> {
    ...
    return (await this.database.imports.toArray()).sort((a, b) => b.importedAt.localeCompare(a.importedAt) || b.id.localeCompare(a.id));
  }
  ...
  public exportRepertoirePgn(repertoireId: string): Promise<{ pgn: string; warnings: string[] }> {
    return this.enqueue(async () => {
      ...
      if (moves.some((move) => move.note && move.purpose)) warnings.push('PGN cannot preserve move note and purpose as distinct structured fields; JSON backup remains lossless.');
      if (contexts.some((row) => row.note)) warnings.push('Context-level notes may not be losslessly representable in PGN.');
      if ((await this.database.managedOpeningNames.toArray()).some((row) => row.repertoireId === repertoireId && !row.archivedAt)) warnings.push('Path-specific opening-name recall metadata and scheduling are not represented in PGN.');
      return { pgn, warnings };
    });
  }
  public clearUserData(confirmation: string, now = nowIso()): Promise<void> {
    this.assertWritable();
    if (confirmation !== 'RESET LOCAL DATA') return Promise.reject(new Error('Reset confirmation did not match.'));
    return this.enqueue(async () => {
      await this.assertMutationUnlocked((await this.database.repertoires.toArray()).map((row) => row.id), (await this.database.playlists.toArray()).map((row) => row.id));
...
export type { MoveSessionOptions, LegacyMoveRecovery } from './phase6RepositoryTraining';
