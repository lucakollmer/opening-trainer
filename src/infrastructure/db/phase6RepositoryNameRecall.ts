...
export class Phase6NameRecallRepository extends Phase6TrainingRepository {
  protected async eligibleNameItems(
    scope: TrainingScope,
    now: Date,
  ): Promise<NameTrainingItemRecord[]> {
    const { graph, playlist, availableIds } = await this.scopeGraph(scope);
    const contexts = new Map(graph.contexts.map((row) => [row.id, row]));
    const names = new Map(
      (await this.database.managedOpeningNames.toArray()).map((row) => [
        row.contextId,
        row,
      ]),
    );
  ...
  private async namePrompt(session: NameSessionRecord): Promise<NamePrompt> {
    ...
    if (!context || !position || !repertoire) {
      throw new Error('Name prompt graph state is missing.');
    }
  ...
  public resumeNameSession(id: string): Promise<NamePrompt> {
    return this.enqueue(async () => {
      await this.assertNoActiveRecallSession({ kind: 'name', id });
      const session = await this.database.nameSessions.get(id);
      if (!session || session.status !== 'active') {
        throw new Error('Interrupted name session is unavailable.');
      }
      return this.namePrompt(session);
    });
  }
  ...
  public reviewName(
    ...
    return this.enqueue(async () => {
      ...
      if (
        ...
        priorState.mappingPolicyVersion !== OPENING_NAME_MAPPING_POLICY_VERSION
      ) {
        throw new Error(
          'Opening-name scheduler configuration is incompatible with this build.',
        );
      }
      ...
      const nextSession: NameSessionRecord = {
        ...session,
      ...
      await this.database.transaction(
        'rw',
        [
          this.database.nameReviewLogs,
          this.database.nameSchedulerStates,
          this.database.nameSchedulerDecisions,
          this.database.nameSessions,
        ],
        async () => {
  ...
  public abandonNameSession(id: string, now = nowIso()): Promise<void> {
    ...
    return this.enqueue(async () => {
      ...
      await this.database.nameSessions.put({
        ...session,
        status: 'abandoned',
        updatedAt: now,
        completedAt: now,
